...
if(CACHE) {
	...
	if(empty($long_url) || !preg_match('|^https?://|', $long_url)) {
	        ...
	        $long_url = mysqli_query($conn, 'SELECT long_url FROM ' . DB_TABLE . ' WHERE id="' . mysqli_real_escape_string($conn, $shortened_id) . '"');
	        $result = mysqli_fetch_array($long_url);
		$long_url = $result[0];
...
if(TRACK) {
	mysqli_query($conn, 'UPDATE ' . DB_TABLE . ' SET referrals=referrals+1 WHERE id="' . mysqli_real_escape_string($conn, $shortened_id) . '"');
}
